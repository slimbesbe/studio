...
import { 
  ...
  TrendingUp, Clock, Calendar, Loader2, ChevronLeft, ChevronRight
} from 'lucide-react';
              ...
              {historyData.length > 0 && (
                <div className="space-y-8 animate-slide-up">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-black italic uppercase tracking-tight text-slate-900">Performance & Évolution</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      ...
                      <Table>
                        <TableHeader className="bg-slate-50">
                          <TableRow className="h-14 border-b-2">
                            ...
                            <TableHead className="font-black uppercase text-[10px] tracking-widest text-center">Ratio</TableHead>
                        ...
                        <TableBody>
                          {historyData.slice().reverse().slice(0, 5).map((a) => (
                            <TableRow key={a.id} className="h-16 border-b last:border-0 group hover:bg-slate-50 transition-colors">
                              <TableCell className="px-6 font-bold italic text-sm text-slate-600">{a.date}</TableCell>
                              <TableCell className="text-center font-black italic text-slate-400">{a.responses}</TableCell>
                              <TableCell className="px-6 text-right font-black italic text-primary">{a.score}%</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                    ...
                    <Card className="rounded-[32px] border-none shadow-xl bg-white p-8">
                      ...
                      <div className="flex justify-center gap-4 mt-6">
                        <div className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-[#ef4444]" /><span className="text-[9px] font-black uppercase text-slate-400">Insuffisant</span></div>
                        <div className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-[#f59e0b]" /><span className="text-[9px] font-black uppercase text-slate-400">Passable</span></div>
                        <div className="flex items-center gap-1.5"><div className="h-2 w-2 rounded-full bg-[#10b981]" /><span className="text-[9px] font-black uppercase text-slate-400">Maîtrisé</span></div>
                      </div>
                    </Card>
                  </div>
                </div>
  ...
  return (
    <div className="perspective-1000 h-48 w-full cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
      <div className={cn("relative w-full h-full transition-transform duration-500 preserve-3d", isFlipped ? "rotate-y-180" : "")}>
        <div className="absolute inset-0 backface-hidden bg-[#1E293B] text-white rounded-[24px] flex flex-col items-center justify-center p-6 shadow-xl border-4 border-slate-700">
...
function QuickQuiz({ questions, axisId, userId, db }: any) {
  ...
  if (showResult) return (
    <Card className="rounded-[40px] bg-white p-12 text-center space-y-8 max-w-xl mx-auto shadow-2xl border-4 border-slate-50">
      <div className="bg-primary/5 h-20 w-24 rounded-3xl flex items-center justify-center mx-auto"><Trophy className="h-10 w-10 text-primary" /></div>
      <h3 className="text-4xl font-black italic uppercase tracking-tighter text-slate-900">Score : {score} / {activeQuestions.length}</h3>
      <Button onClick={() => window.location.reload()} className="h-16 px-10 rounded-2xl bg-primary hover:bg-primary/90 font-black uppercase tracking-widest text-lg shadow-xl w-full">Terminer & Analyser</Button>
    </Card>
  );
  ...
  return (
    <Card className="rounded-[40px] bg-white p-10 space-y-8 max-w-3xl mx-auto shadow-2xl border-4 border-slate-50">
      <div className="flex justify-between items-center">
        <Badge variant="outline" className="font-black italic px-4 py-1 rounded-xl border-2 text-slate-400">Question {currentIdx + 1} / {activeQuestions.length}</Badge>
      </div>
      ...
      <div className="grid gap-4">
        {q.a.map((opt: any, idx: number) => (
          <button 
            ...
            className={cn(
              "p-6 rounded-2xl border-2 transition-all text-left font-bold italic flex items-center justify-between shadow-sm", 
              !isAnswered ? "border-slate-100 hover:border-primary hover:bg-slate-50" : 
              idx === q.c ? "border-emerald-500 bg-emerald-50 text-emerald-900" : 
              idx === selectedIdx ? "border-red-500 bg-red-50 text-red-900" : "opacity-50 border-slate-50"
            )}
          >
            <span>{opt}</span>
            {isAnswered && idx === q.c && <CheckCircle2 className="h-6 w-6 text-emerald-500" />}
            {isAnswered && idx === selectedIdx && idx !== q.c && <XCircle className="h-6 w-6 text-red-500" />}
          </button>
        ))}
      </div>
      {isAnswered && (
        <div className="bg-slate-50 p-8 rounded-[24px] border-l-8 border-l-primary animate-slide-up shadow-inner">
          <div className="flex items-center gap-2 mb-3"><Info className="h-4 w-4 text-primary" /><span className="text-[10px] font-black uppercase italic text-primary">Justification Mindset</span></div>
          <p className="text-slate-700 font-bold italic text-base leading-relaxed">{q.exp}</p>
          <Button onClick={next} className="mt-8 w-full h-14 rounded-2xl bg-slate-900 text-white font-black uppercase italic tracking-widest text-xs shadow-lg">Suivant</Button>
        </div>
      )}
    </Card>
  );

    ...
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in space-y-2 h-[calc(100vh-80px)] box-border p-2">
      ...
      <div className="shrink-0 flex justify-between items-end">
        <div className="space-y-0.5">
          <h1 className="text-xl font-black italic uppercase tracking-tighter text-slate-900 leading-none">Vision Domaines</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[8px] italic">Les 3 piliers du PMP® en un coup d'œil.</p>
        </div>
      </div>
        ...
        {(['people', 'process', 'business'] as const).map((id) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("font-black uppercase italic tracking-widest text-[8px]", isActive ? "text-primary" : "text-slate-400")}>{id.toUpperCase()}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="shrink-0 flex items-center justify-between bg-white/50 p-1.5 rounded-xl border border-slate-100">
        <div className="flex items-center gap-3">
          <h2 className="text-[11px] font-black italic uppercase tracking-tight text-slate-900 ml-1">{currentData.title}</h2>

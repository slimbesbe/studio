...
export default function VisionDomainesPage() {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in space-y-4 h-[calc(100vh-120px)] box-border">
      {/* HEADER COMPACT */}
      <div className="shrink-0 space-y-1">
        <h1 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900 leading-none">Vision Domaines</h1>
        <p className="text-slate-500 font-bold uppercase tracking-widest text-[9px] italic">Explorez les 3 piliers de l'examen PMP®.</p>
      </div>

      {/* SELECTEUR DE DOMAINE COMPACT */}
      <div className="shrink-0 grid grid-cols-3 gap-3 h-20">
          ...
          return (
            <button 
              ...
              className={cn(
                ...
                isActive ? "border-primary scale-[1.02] bg-primary/5" : "border-slate-100 hover:border-slate-200"
              )}
            >
      ...
      {/* CONTROLES ET TABS */}
      <div className="shrink-0 flex items-center justify-between bg-white/50 p-2 rounded-2xl border border-slate-100">
        <div className="flex items-center gap-4">
          <h2 className="text-sm font-black italic uppercase tracking-tight text-slate-900 ml-2">{currentData.title}</h2>
          <div className="flex bg-slate-100 p-1 rounded-xl shadow-inner border">
            <Button 
              size="sm"
              onClick={() => setActiveTab('jargon')} 
              className={cn("h-7 px-4 rounded-lg font-black uppercase italic text-[9px] gap-2", activeTab === 'jargon' ? "bg-slate-900 text-white shadow-md" : "bg-transparent text-slate-500 hover:bg-slate-200")}
            >
            ...
            <Button 
              ...
              className={cn("h-7 px-4 rounded-lg font-black uppercase italic text-[9px] gap-2", activeTab === 'quiz' ? "bg-slate-900 text-white shadow-md" : "bg-transparent text-slate-500 hover:bg-slate-200")}
            >
        ...
        {activeTab === 'jargon' && totalPages > 1 && (
          <div className="flex items-center gap-3">
            <Button 
              variant="outline" 
              size="icon" 
              className="h-8 w-8 rounded-full border-2 shadow-sm bg-white"
              disabled={currentPage === 0}
              onClick={() => setCurrentPage(currentPage - 1)}
            >
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <span className="text-[9px] font-black italic text-slate-400 uppercase tracking-widest">{currentPage + 1} / {totalPages}</span>
            <Button 
              variant="outline" 
              size="icon" 
              className="h-8 w-8 rounded-full border-2 shadow-sm bg-white"
      ...
      <div className="flex-1 min-h-0 overflow-hidden pb-4">
        {activeTab === 'jargon' ? (
          <div className="grid grid-cols-1 md:grid-cols-3 grid-rows-2 gap-4 h-full animate-slide-up">
            ...
            {/* Filler pour garder la structure 3x2 même si moins de 6 items */}
            {paginatedJargon.length < 6 && Array.from({ length: 6 - paginatedJargon.length }).map((_, i) => (
              <div key={`filler-${i}`} className="bg-slate-50/30 border-2 border-dashed border-slate-100 rounded-[24px]" />
            ))}
          </div>
        ) : (
  ...
  return (
    <div className="perspective-1000 h-full w-full cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
      <div className={cn("relative w-full h-full transition-transform duration-500 preserve-3d", isFlipped ? "rotate-y-180" : "")}>
        ...
        <div className="absolute inset-0 backface-hidden bg-[#f0fdf4] text-emerald-900 rounded-[28px] flex flex-col items-center justify-center p-6 shadow-lg border-2 border-emerald-100">
          <h3 className="text-base font-black italic uppercase tracking-tight text-center leading-tight">{term}</h3>
        </div>
        {/* VERSO : Gris Anthracite profond */}
        <div className="absolute inset-0 backface-hidden rotate-y-180 bg-[#1e293b] text-white rounded-[28px] flex items-center justify-center p-6 shadow-2xl border-2 border-slate-700">
          <p className="text-center font-bold italic text-xs leading-relaxed">{def}</p>
        </div>
      </div>
    </div>
  );
...
function QuickQuiz({ questions, axisId, userId, db }: any) {
  ...
  if (showResult) return (
    <Card className="rounded-[40px] bg-white p-10 text-center space-y-6 shadow-2xl animate-fade-in border-none w-full max-w-md">
      <div className="bg-primary/5 w-16 h-16 rounded-[24px] flex items-center justify-center mx-auto shadow-inner">
      ...
      <div className="space-y-1">
        ...
        <p className="text-slate-400 font-bold uppercase tracking-widest text-[8px] italic">Session validée</p>
      </div>
      <Button onClick={() => window.location.reload()} className="h-12 px-10 rounded-xl bg-primary font-black uppercase tracking-widest shadow-xl text-white text-xs">REFAIRE</Button>
    </Card>
  );
  ...
  return (
    <Card className="rounded-[32px] bg-white p-6 space-y-4 w-full max-w-xl shadow-2xl animate-slide-up border-none overflow-hidden">
      <Badge variant="outline" className="font-black italic px-3 py-0.5 rounded-lg border-2 text-slate-400 text-[9px]">
        Question {currentIdx + 1} / {activeQuestions.length}
      </Badge>
      <h3 className="text-base font-black italic text-slate-900 leading-tight">
        {q.q || q.text}
      </h3>
      <div className="grid gap-2">
        {rawChoices.map((opt: any, idx: number) => (
          ...
          >
            <div className={cn(
              "h-6 w-6 flex items-center justify-center font-black text-[10px] shrink-0 border-2 rounded-full",
            ...
            <span className="flex-1 text-sm font-black italic pt-0.5">{opt}</span>
          </button>
        ))}
      </div>
      {isAnswered && (
        <div className="bg-slate-50 p-4 rounded-[20px] border-l-4 border-l-primary animate-slide-up space-y-3">
          <p className="text-black font-bold italic text-xs leading-relaxed">{q.exp || q.explanation}</p>
          <Button onClick={next} className="w-full h-9 rounded-lg bg-slate-900 text-white font-black uppercase italic tracking-widest text-[9px]">

  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setCurrentPage(0); // Reset on domain change
      }
    }
    load();
        ...
        {(['people', 'process', 'business'] as const).map((id) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("font-black uppercase italic tracking-widest text-[10px]", isActive ? "text-primary" : "text-slate-400")}>{id.toUpperCase()}</span>
            </button>
          );
        })}
      </div>

    ...
    <div className="flex-1 flex flex-col h-full w-full overflow-hidden animate-fade-in p-[1vh] gap-[1vh] bg-background">
      ...
      <header className="flex-none flex justify-between items-end px-2 h-[5vh]">
        <div className="flex flex-col">
          <h1 className="text-[clamp(1.2rem,3vh,2.5rem)] font-black italic uppercase tracking-tighter text-slate-900 leading-none">Vision Domaines</h1>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[clamp(0.5rem,1.2vh,0.8rem)] italic leading-none mt-1">Les 3 piliers du PMP® en un coup d'œil.</p>
        </div>
      </header>
        ...
        {(['people', 'process', 'business'] as const).map((id) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn("font-black uppercase italic tracking-widest text-[clamp(0.6rem,1.4vh,1rem)]", isActive ? "text-primary" : "text-slate-400")}>{id.toUpperCase()}</span>
            </button>
          );
        })}
      </nav>
      ...
      <div className="flex-none flex items-center justify-between bg-white px-4 rounded-2xl border border-slate-100 shadow-sm h-[7vh]">
        <div className="flex items-center gap-[2vw] flex-1">
          ...
          <h2 className="text-[clamp(0.8rem,1.8vh,1.2rem)] font-black italic uppercase tracking-tight text-slate-400 truncate hidden lg:block">{currentData.title}</h2>
        </div>

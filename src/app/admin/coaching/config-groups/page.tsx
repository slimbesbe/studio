    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black italic uppercase tracking-tighter text-primary">Configuration Groupes</h1>
            <p className="text-muted-foreground mt-1 uppercase tracking-widest text-[10px] font-bold italic">Créez vos cohortes et gérez les participants.</p>
          </div>
        </div>
        <Button onClick={() => setIsModalOpen(true)} className="bg-indigo-600 h-14 px-8 rounded-2xl font-black uppercase tracking-widest shadow-xl scale-105 transition-transform hover:bg-indigo-700">
              ...
              {groups?.map((g) => (
                <TableRow key={g.id} className="h-24 hover:bg-slate-50 transition-all border-b last:border-0 group">
                  ...
                  <TableCell className="text-right px-10">
                    <Button variant="ghost" asChild className="h-12 w-12 rounded-2xl hover:bg-indigo-50 border-2 border-indigo-100 text-indigo-600">
                      <Link href={`/admin/coaching/config-groups/${g.id}`} title="Gérer les participants"><ArrowRight className="h-6 w-6" /></Link>
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="rounded-[40px] p-12 border-4 shadow-3xl">
          <DialogHeader>
            <DialogTitle className="text-3xl font-black uppercase italic tracking-tighter text-indigo-600">Nouveau Groupe</DialogTitle>
            <DialogDescription className="font-bold text-slate-500 italic mt-2 uppercase text-xs tracking-widest">Identifiez votre cohorte de coaching.</DialogDescription>
          </DialogHeader>
          <div className="py-8 space-y-4">
            <Label className="font-black uppercase text-xs text-slate-400">Nom de la cohorte (ex: Groupe Octobre 2023)</Label>
            <Input 
              value={newGroupName} 
          ...
          <DialogFooter className="gap-4">
            ...
            <Button onClick={handleCreateGroup} disabled={isSubmitting || !newGroupName.trim()} className="h-16 rounded-2xl font-black bg-indigo-600 hover:bg-indigo-700 flex-1 shadow-2xl uppercase">

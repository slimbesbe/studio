                ...
                {data.quiz?.map((q:any, idx:number) => (
                  <div key={idx} className="p-6 bg-amber-50/30 rounded-3xl border-2 border-amber-100 relative group space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label className="text-[9px] font-black uppercase text-amber-600 italic">Question ?</Label>
                      <Input placeholder="Votre question..." value={q.q} onChange={(e) => updateQuizField(idx, 'q', e.target.value)} className="h-10 bg-white rounded-lg font-black italic border-2 border-amber-200" />
                    </div>

                    <div className="space-y-3">
                      <div className="flex justify-between items-center">
                        <Label className="text-[9px] font-black uppercase text-amber-600 italic">Choix possibles</Label>
                      ...
                      <div className="grid gap-2">
                        {q.a.map((opt:string, optIdx:number) => (
                          <div key={optIdx} className="flex items-center gap-2 group/choice">
                            ...
                            <Input placeholder={`Option ${String.fromCharCode(65 + optIdx)}`} value={opt} onChange={(e) => updateQuizChoice(idx, optIdx, e.target.value)} className={cn("h-10 bg-white rounded-lg font-bold italic text-sm border-2", q.c === optIdx ? "border-amber-400" : "border-amber-100")} />
                    ...
                    <div className="space-y-2">
                      <Label className="text-[9px] font-black uppercase text-amber-600 italic">Justification Mindset</Label>
                      <Textarea placeholder="Justification..." value={q.exp} onChange={(e) => updateQuizField(idx, 'exp', e.target.value)} className="h-20 bg-white rounded-lg font-bold italic text-xs border-2 border-amber-100" />
                    </div>
                  </div>
                ))}
              </div>

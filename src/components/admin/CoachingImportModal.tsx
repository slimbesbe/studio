...
export function CoachingImportModal({ isOpen, onClose, session }: CoachingImportModalProps) {
  ...
  const parseFile = async (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const count = session.questionEnd - session.questionStart + 1;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      await batch.commit();
      toast({ title: "Simulation importée", description: `${parsedData.length} questions ajoutées.` });
      onClose();
      setFile(null);
    } catch (e) {
      ...
      <DialogContent className="max-w-2xl rounded-[40px] p-10 border-4 shadow-3xl">
        ...
        <div className="py-6 space-y-6">
          {!file ? (
            ...
            >
              <Upload className="h-12 w-12 mx-auto text-slate-300 group-hover:text-emerald-500 mb-4" />
              ...
              <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl border-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase">{parsedData.length} questions prêtes</p>
                  </div>
                </div>
                <Button variant="ghost" size="sm" className="font-black uppercase text-[10px]" onClick={() => { setFile(null); setParsedData([]); }}>Changer</Button>
              </div>
              {isParsing && <div className="flex flex-col items-center py-4 gap-2"><Loader2 className="animate-spin h-8 w-8 text-primary" /><p className="font-black text-[10px] uppercase text-slate-400 italic">Analyse...</p></div>}
            </div>
          )}
          {isImporting && <div className="space-y-2"><Progress value={progress} className="h-3 rounded-full" /><p className="text-[10px] text-center font-black uppercase italic text-slate-400">Import en cours...</p></div>}
        </div>
        ...
        <DialogFooter className="gap-4">
          ...
          <Button disabled={parsedData.length === 0 || isImporting || isParsing} onClick={handleImport} className="h-14 rounded-xl font-black bg-emerald-600 hover:bg-emerald-700 flex-1 shadow-2xl uppercase">
            {isImporting ? <Loader2 className="animate-spin h-5 w-5" /> : "Lancer l'Import"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
